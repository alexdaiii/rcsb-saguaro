...
> {
  ...
  componentDidMount() {
    ...
    const refDiv: HTMLDivElement | null = document.querySelector(
      "#" + this.props.boardId,
    );
    if (refDiv == null) throw "Main board DOM element not found";
    this.refDiv = refDiv;
    const tooltipDiv: HTMLDivElement | null = document.querySelector(
      "#" + this.props.boardId + RcsbFvDOMConstants.UI_DOM_ID_PREFIX,
    );
    if (tooltipDiv == null) throw "Tooltip DOM element not found";
  ...
  private displayUI(): void {
    if (this.hideTask) this.hideTask.unsubscribe();

    const offsetHeight: number =
      this.props.boardConfigData.includeAxis === true
        ? RcsbFvDefaultConfigValues.trackAxisHeight + 2
        : 0;
  ...
  private zoomIn(): void {
    const max: number | undefined =
      this.props.boardConfigData.range != null
        ? this.props.boardConfigData.range.max
        : this.props.boardConfigData.length;
    if (max == null) return;

    const currentDomain: Array<number> = this.props.xScale.domain();
    const deltaZoom: number = Math.floor(
      (currentDomain[1] - currentDomain[0]) * 0.1,
    );
  ...
  private zoomOut(): void {
    ...
    const x: number =
      currentDomain[0] - deltaZoom >
      min - RcsbFvDefaultConfigValues.increasedView
        ? currentDomain[0] - deltaZoom
        : min - RcsbFvDefaultConfigValues.increasedView;
    const y: number =
  ...
  private move(direction: 1 | -1): void {
    ...
    const currentDomain: Array<number> = this.props.xScale.domain();
    let deltaZoom: number = Math.floor(
      (currentDomain[1] - currentDomain[0]) * 0.1,
    );
    if (
      currentDomain[0] + direction * deltaZoom <
      min - RcsbFvDefaultConfigValues.increasedView
    )
      deltaZoom =
        currentDomain[0] - (min - RcsbFvDefaultConfigValues.increasedView);
    else if (
      currentDomain[1] + direction * deltaZoom >
      max + RcsbFvDefaultConfigValues.increasedView
    )
      deltaZoom =
        max + RcsbFvDefaultConfigValues.increasedView - currentDomain[1];
