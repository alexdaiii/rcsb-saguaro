...
> {
  ...
  private readonly xScale: RcsbScaleInterface =
    RcsbD3ScaleFactory.getLinearScale();
  ...
  constructor(props: RcsbFvInterface<P, S, R, M>) {
    ...
    if (this.boardConfigData != null) {
      this.init().then(() => {
        console.info(
          `PFV ${this.elementId} is ready. Configuration provided during object instantiation`,
        );
      });
    }
  }
   ...
   */
  public setBoardData(
    rowConfigData: RcsbFvInterface<P, S, R, M>["rowConfigData"],
  ): Promise<void> {
  ...
  public then(f: () => void): RcsbFv {
    ...
    else throw "RcsbFv init method was not called";
    return this;
  }
  ...
  public init(): Promise<void> {
    this.rcsbFvPromise = new Promise<void>((resolve, reject) => {
      if (!this.mounted && this.boardConfigData != undefined) {
        ...
        reject(
          "FATAL ERROR: RcsvFvBoard is mounted or board configuration was not loaded",
        );
      }
    });
   ...
   * */
  public updateBoardConfig(newConfig: {
    ...
    else this.boardDataSate.refresh();

    if (newConfig.boardConfigData)
      this.boardConfigData = {
        ...this.boardConfigData,
        ...newConfig.boardConfigData,
      };
   ...
   * */
  public addTrack(
    trackConfig: RcsbFvRowConfigInterface<P, S, R, M>,
  ): Promise<void> {
