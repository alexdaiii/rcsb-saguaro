...
export class RcsbFvDisplay {
  ...
  public initDisplay(): RcsbDisplayInterface {
    ...
    if (
      typeof config.displayType === "string" &&
      config.displayType != RcsbFvDisplayTypes.COMPOSITE
    ) {
      const out: RcsbDisplayInterface | null = RcsbFvDisplay.singleDisplay(
        config.displayType,
        config,
      );
      if (out != null) return out;
      else throw "Display config " + config + " not supported";
    } else if (
      typeof config.displayType === "string" &&
      config.displayType == RcsbFvDisplayTypes.COMPOSITE
    ) {
    ...
    if (config.displayConfig != undefined)
      for (let displayItem of config.displayConfig) {
        let displayId: string =
          "displayId_" + Math.random().toString(36).substr(2);
        ...
        if (config.displayConfig) {
          displayConfig = RcsbFvDisplay.setDisplayConfig(
            config,
            config.displayConfig[i],
          );
          i++;
        }
        const singleDisplay: RcsbDisplayInterface | null =
          RcsbFvDisplay.singleDisplay(displayType, displayConfig);
  ...
  private static singleDisplay(
    ...
    if (
      ...
      config.trackId != undefined &&
      config.displayColor != undefined
    ) {
      switch (type) {
        ...
        case RcsbFvDisplayTypes.PIN:
          ...
          else throw "Track displayDomain (yScale) not defined";
          break;
        ...
        case RcsbFvDisplayTypes.SEQUENCE:
          const dynamicDisplay: boolean =
            config.dynamicDisplay != undefined ? config.dynamicDisplay : false;
          const nonEmptyDisplay: boolean =
            config.nonEmptyDisplay != undefined
              ? config.nonEmptyDisplay
              : false;
          out = sequenceDisplay(
            config.boardId,
            config.trackId,
        ...
        case RcsbFvDisplayTypes.AREA:
          if (config.displayDomain != undefined)
            out = areaDisplay(
              config.boardId,
              config.trackId,
              config.displayColor as string | RcsbFvColorGradient,
              config.displayDomain,
              config.interpolationType,
            );
          else throw "Track displayDomain (yScale) not defined";
...
function sequenceDisplay(
  ...
  nonEmptyDisplayFlag: boolean,
): RcsbDisplayInterface {
  const display: RcsbFastSequenceDisplay = new RcsbFastSequenceDisplay(
    boardId,
    trackId,
  );
...
function areaDisplay(
  ...
  domain: [number, number],
  interpolationType?: string,
  blockAreaFlag?: boolean,
  multiAreaFlag?: boolean,
): RcsbDisplayInterface {
  ...
  if (typeof interpolationType === "string")
    display.setInterpolationType(interpolationType);
  if (typeof blockAreaFlag === "boolean") display.setBlockArea(blockAreaFlag);
  if (typeof multiAreaFlag === "boolean") display.setMultiArea(multiAreaFlag);
  return display;
}
