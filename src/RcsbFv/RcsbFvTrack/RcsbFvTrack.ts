...
export class RcsbFvTrack {
  ...
  private rcsbTrackArray: Array<RcsbDisplayInterface> =
  ...
  public constructor(
    ...
    if (
      ...
      document.getElementById(args.elementId) != null
    ) {
      this.rcsbBoard = new RcsbBoard(
        args.elementId,
        xScale,
        this.selection,
        this.contextManager,
  ...
  private buildTrack(args: RcsbFvRowExtendedConfigInterface): void {
    ...
    if (
      typeof this.rcsbFvConfig.trackData != "undefined" &&
      this.rcsbFvConfig.displayType != RcsbFvDisplayTypes.COMPOSITE
    ) {
  ...
  private initRcsbBoard(): void {
    if (typeof this.rcsbFvConfig.elementClickCallback === "function")
      this.rcsbBoard.setElementClickCallback(
        this.rcsbFvConfig.elementClickCallback,
      );
    ...
    if (typeof this.rcsbFvConfig.range === "object")
      this.rcsbBoard.setRange(
        this.rcsbFvConfig.range.min - RcsbFvDefaultConfigValues.increasedView,
        this.rcsbFvConfig.range.max + RcsbFvDefaultConfigValues.increasedView,
      );
    else if (typeof this.rcsbFvConfig.length === "number")
      this.rcsbBoard.setRange(
        1 - RcsbFvDefaultConfigValues.increasedView,
        this.rcsbFvConfig.length + RcsbFvDefaultConfigValues.increasedView,
      );
  }
  ...
  private load(trackData: RcsbFvTrackData | Array<RcsbFvTrackData>): void {
    if (
      ...
      const trackNonOverlappingMap: Array<Array<RcsbFvTrackData>> = new Array<
        Array<RcsbFvTrackData>
      >();
      let maxTracks: number = 1;
      (trackData as Array<RcsbFvTrackData>).forEach((f, i) => {
        if (!this.rcsbFvConfig.overlap) {
          const nonOverlapping: Array<RcsbFvTrackData> =
            RcsbDataManager.getNonOverlappingData(f);
      ...
      for (let i = 0; i < maxTracks; i++) {
        ...
        if (this.rcsbFvConfig?.trackHeight != undefined)
          (rcsbCompositeTrack as RcsbCompositeDisplay).setCompositeHeight(
            i * this.rcsbFvConfig.trackHeight,
          );
        ...
        trackNonOverlappingMap.forEach((v, j) => {
          ...
          if (i < v.length) trackDataMap.set(id, v[i]);
          else trackDataMap.set(id, []);
        });
  ...
  public getTrackHeight(): number | null {
    if (
      this.rcsbTrackArray.length > 0 &&
      this.rcsbFvConfig?.trackHeight != undefined
    ) {
