...
export class RcsbFvConfig implements RcsbFvRowExtendedConfigInterface {
  ...
  public updateConfig(args: RcsbFvRowExtendedConfigInterface): void {
    ...
    if (
      ...
      typeof args.range.min === "number" &&
      typeof args.range.max === "number"
    ) {
    ...
    if (
      typeof args.displayColor === "string" ||
      (args.displayColor != null && typeof args.displayColor === "object")
    ) {
    ...
    if (typeof args.interpolationType === "string") {
      this.interpolationType = this.getInterpolationType(
  ...
  configCheck(): boolean {
    return (
      (typeof this.length === "number" || typeof this.range === "object") &&
