...
export class RowGlow extends React.Component<RowGlowInterface> {
  ...
  render() {
    return (
      <div
        id={this.props.boardId + RcsbFvDOMConstants.GLOW_ROW_DOM_ID_SUFFIX}
        className={classes.rcsbNoRowGlow}
      >
        <div
          style={{
            borderWidth: RcsbFvDefaultConfigValues.rowGlowWidth,
            borderColor: RcsbFvDefaultConfigValues.rowGlowColor,
          }}
        />
      </div>
    );
  ...
  private displayGlow(id: string): void {
    const boardDiv: HTMLElement | null = document.getElementById(
      this.props.boardId,
    );
    ...
    if (rowDiv != null && boardDiv != null) {
      ...
      const height: number =
        rowDiv.getBoundingClientRect().height -
        2 * RcsbFvDefaultConfigValues.rowGlowWidth;
      const glowDiv: HTMLElement | null = document.getElementById(
        this.props.boardId + RcsbFvDOMConstants.GLOW_ROW_DOM_ID_SUFFIX,
      );
      if (glowDiv != null) {
        const innerGlowDiv: HTMLElement | undefined =
          glowDiv.getElementsByTagName("div")[0];
        const trackWidth: number =
          this.props.boardConfigData.trackWidth ??
          RcsbFvDefaultConfigValues.trackWidth;
        const titleWidth: number =
          this.props.boardConfigData.rowTitleWidth ??
          RcsbFvDefaultConfigValues.rowTitleWidth;
        glowDiv.style.top = top + "px";
        glowDiv.style.marginLeft =
          titleWidth + RcsbFvDefaultConfigValues.titleAndTrackSpace + "px";
