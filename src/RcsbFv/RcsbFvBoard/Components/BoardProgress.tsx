...
export class BoardProgress extends React.Component<BoardProgressInterface> {
  ...
  render() {
    return (
      <div
        id={this.props.boardId + RcsbFvDOMConstants.PROGRESS_DIV_DOM_ID_PREFIX}
        style={{
          position: "absolute",
          top: 0,
          left: 0,
        ...
        className={classes.rowTrackBoardSatus}
      >
        LOADING <span />
      </div>
    );
  ...
  componentDidMount(): void {
    ...
    const tooltipDiv: HTMLDivElement | null = document.querySelector(
      "#" + this.props.boardId + RcsbFvDOMConstants.PROGRESS_DIV_DOM_ID_PREFIX,
    );
  ...
  private rowReady(fraction: number): void {
    if (fraction >= 100) {
      ...
      if (this.tooltipDiv.style.visibility == "hidden") this.showStatus();
      const statusDiv: HTMLElement | null = document.querySelector(
        "#" +
          this.props.boardId +
          RcsbFvDOMConstants.PROGRESS_DIV_DOM_ID_PREFIX +
          " > span",
      );
      if (statusDiv != null) statusDiv.innerHTML = fraction.toString() + "%";
    }
  }
