...
> {
  ...
  render(): ReactNode {
    ...
    return (
      <div
        className={classes.rcsbFvRootContainer}
        onMouseOver={this.setMouseOverCallback()}
        onMouseLeave={this.setMouseLeaveCallback()}
      >
        {this.state.boardConfigData.disableMenu ? null : (
          <RcsbFvUI
        ...
        />
        <div
          id={this.props.boardId + RcsbFvDOMConstants.TOOLTIP_DOM_ID_PREFIX}
          className={classes.rcsbFvTooltip}
          {...{[RcsbFvDOMConstants.POPPER_HIDDEN]: ""}}
        />
        <div
          id={
            this.props.boardId +
            RcsbFvDOMConstants.TOOLTIP_DESCRIPTION_DOM_ID_PREFIX
          }
          className={classes.rcsbFvTooltipDescription}
          {...{[RcsbFvDOMConstants.POPPER_HIDDEN]: ""}}
        />
        <BoardProgress
          boardId={this.props.boardId}
  ...
  componentDidUpdate(
    prevProps: Readonly<RcsbFvBoardInterface>,
    prevState: Readonly<RcsbFvBoardState>,
    snapshot?: any,
  ) {
  ...
  private subscribe(): Subscription {
    return this.props.contextManager.subscribe(
      (obj: RcsbFvContextManagerType) => {
  ...
  private updateBoardConfig(
    configData: Partial<RcsbFvBoardFullConfigInterface>,
  ): void {
    if (
      configData.rowConfigData != null &&
      configData.boardConfigData != null
    ) {
      this.setState({
  ...
  private setMouseOverCallback(): (() => void) | undefined {
    if (this.state.boardConfigData.hideTrackFrameGlow) return undefined;
    else return this.onMouseOver.bind(this);
  }

  private setMouseLeaveCallback(): (() => void) | undefined {
    if (this.state.boardConfigData.hideTrackFrameGlow) return undefined;
    else return this.onMouseLeave.bind(this);
  }
  ...
  private renderStarts(): void {
    if (
      typeof this.state.boardConfigData.onFvRenderStartsCallback === "function"
    )
      this.state.boardConfigData.onFvRenderStartsCallback();

    if (
      typeof this.state.boardConfigData.selectionChangeCallback === "function"
    )
      this.selection.setSelectionChangeCallback(
        this.state.boardConfigData.selectionChangeCallback,
      );
  }
