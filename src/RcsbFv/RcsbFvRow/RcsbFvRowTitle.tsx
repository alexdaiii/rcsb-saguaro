...
export type RowTitleComponentType<P = {}, S = {}> = typeof React.Component<
  RcsbFvRowTitleInterface & P,
  S
>;

export class RcsbFvRowTitle extends React.Component<
  RcsbFvRowTitleInterface,
  RcsbFvRowTitleInterState
> {
  ...
  public render(): ReactNode {
    ...
    const trackTitle: string =
      typeof this.configData?.rowTitle === "string"
        ? this.configData.rowTitle
        : typeof this.configData?.rowTitle === "object"
          ? this.configData.rowTitle.visibleTex
          : "";

    let titleElement: ReactNode;
    if (this.props.data.externalRowTitle?.rowTitleComponent) {
      const rowTitleProps =
        this.props.data.externalRowTitle.rowTitleAdditionalProps;
      const RowTitleComponent: RowTitleComponentType<
        typeof rowTitleProps,
        any
      > = this.props.data.externalRowTitle?.rowTitleComponent;
      titleElement = (
        <>
          <div
            className={
              classes.rcsbFvRowTitleText +
              (this.state.expandTitle
                ? " " + classes.rcsbFvRowTitleTextExpand
                : "")
            }
            style={{
            ...
            onMouseEnter={(evt) => {
              this.expandTitle(evt, true);
            }}
            onMouseLeave={(evt) => {
              this.expandTitle(evt, false);
            }}
          >
            <RcsbFvRowMark
      ...
      typeof this.configData.rowPrefix === "string" &&
      this.configData.rowPrefix.length > 0 &&
      this.configData.fitTitleWidth
    ) {
      const prefixLength: number = Math.max(
        this.configData.rowPrefix.length,
        16,
      );
      const prefixWidth: number = Math.round(
        (prefixLength / (prefixLength + trackTitle.length)) *
          (this.configStyle().width as number),
      );
      const titleWidth: number =
        (this.configStyle().width as number) - prefixWidth;
      ...
      <div className={classes.rcsbFvRowTitle} style={this.configStyle()}>
        {this.setTitle() != null ? (
          <div
            ...
            className={classes.rcsbFvRowTitleProvenanceFlag}
          />
        ) : null}
        {titleElement}
      </div>
  ...
  private setTitle(): string | null | ReactNode {
    if (typeof this.configData.rowTitle === "string") {
      ...
      const target: string = this.configData.rowTitle.isThirdParty
        ? "_blank"
        : "_self";
      if (typeof this.configData.rowTitle.url === "string")
        return (
          <a
            href={this.configData.rowTitle.url}
            target={target}
            style={this.configData.rowTitle.style}
          >
            {this.configData.rowTitle.visibleTex}
          </a>
        );
        ...
        return (
          <span style={this.configData.rowTitle.style}>
            {this.configData.rowTitle.visibleTex}
          </span>
        );
    }
