...
export class RcsbLineDisplay extends RcsbAbstractDisplay {
  ...
  protected innerData: Array<RcsbFvTrackDataElementInterface | null> =
    new Array<RcsbFvTrackDataElementInterface | null>();
  ...
  protected line: Line<RcsbFvTrackDataElementInterface> =
    line<RcsbFvTrackDataElementInterface>().curve(curveStep);
  ...
  setScale(): void {
    if (
      ...
      this._yDomain.length == 2 &&
      typeof this._yDomain[0] === "number" &&
      typeof this._yDomain[1] === "number"
    ) {
      this.yScale.domain(this._yDomain).range([this.height() - 3, 3]);
  ...
  protected downSampling(
    ...
    const filterPoints = tmp.filter(
      (p) => p.begin > domain.min && p.begin < domain.max,
    );
