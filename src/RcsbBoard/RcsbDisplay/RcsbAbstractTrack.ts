...
export abstract class RcsbAbstractTrack implements RcsbTrackInterface {
  ...
  protected updateDataOnMove: (
    d: LocationViewInterface,
  ) => Promise<RcsbFvTrackData>;
  ...
  init(
    ...
    if (compositeFlag === true) {
      height = 0;
      if (typeof compositeHeight === "number") compH = compositeHeight;
      else height = 0;
    }
  ...
  highlightRegion(
    ...
    if (d != null) {
      ...
      this.highlightRegionTask = asyncScheduler.schedule(() => {
        this.g
          .selectAll("." + (options?.rectClass ?? classes.rcsbSelectRect))
          .remove();
      }, 10);
    }
  }
  ...
  moveSelection(mode: "select" | "hover"): void {
    ...
    const moveSelectionConfig: MoveSelectedRegionInterface = {
      ...
      xScale: xScale,
      rectClass:
        mode === "select" ? classes.rcsbSelectRect : classes.rcsbHoverRect,
    };
