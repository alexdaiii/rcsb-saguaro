...
export abstract class RcsbAbstractDisplay
  ...
  implements RcsbDisplayInterface
{
  ...
  private elementSelection: Selection<
    ...
    BaseType,
    undefined
  > = select<SVGGElement, RcsbFvTrackDataElementInterface>(
  ...
  public subscribeElementHighlight(action: {
  ...
  plot(
    ...
    element.on(
      RcsbD3Constants.DBL_CLICK,
      (event: MouseEvent, d: RcsbFvTrackDataElementInterface) => {
        if (event.defaultPrevented) return;
      },
    );
    element.on(
      RcsbD3Constants.MOUSE_LEAVE,
      (event: MouseEvent, d: RcsbFvTrackDataElementInterface) => {
  ...
  update(compKey?: string): void {
    const where: LocationViewInterface = {
      from: Math.floor(this.xScale.domain()[0]),
      to: Math.ceil(this.xScale.domain()[1]),
    };
    if (typeof this.updateDataOnMove === "function") {
      this.updateDataOnMove(where)
        .then((result: RcsbFvTrackData) => {
          this.data(result);
          if (this.data() != null) {
  ...
  _update(where: LocationViewInterface, compKey?: string): void {
    if (this.selectDataInRangeFlag) {
      let dataElems: RcsbFvTrackData = this.processData(
        this.data().filter((s: RcsbFvTrackDataElementInterface, i: number) => {
          if (s.end == null) {
      ...
      this.selectElements(dataElems, compKey)
        ...
        .classed(
          classes.rcsbElement + "_" + compKey,
          typeof compKey === "string",
        )
        .call(this.plot.bind(this));
    } else if (
      (this.minRatio == 0 || this.getRatio() > this.minRatio) &&
      !this.isDataUpdated()
    ) {
  ...
  > {
    ...
    const elements: Selection<
      ...
      undefined
    > = this.g
      .selectAll<SVGGElement, RcsbFvTrackDataElementInterface>(className)
      .data(dataElems, RcsbAbstractDisplay.dataKey);
    const newElems: Selection<
      SVGGElement,
      RcsbFvTrackDataElementInterface,
      BaseType,
      undefined
