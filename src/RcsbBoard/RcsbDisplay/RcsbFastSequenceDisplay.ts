...
export class RcsbFastSequenceDisplay extends RcsbAbstractDisplay {
  ...
  private readonly rcsbD3SequenceManager: RcsbD3FastSequenceManager =
    new RcsbD3FastSequenceManager();
  ...
  _update(where: LocationViewInterface, compKey?: string) {
    ...
    if (this.minIntervalRatio()) {
      const dataElems: Array<RcsbFvTrackDataElementInterface> =
        this.getSequenceData(where);
      this.rmSequenceLine();
  ...
  private setScale(): void {
    this.yScale.domain([0, this.height()]).range([0, this.height()]);
  ...
  private getSequenceData(
    ...
    const seqPath: Array<RcsbFvTrackDataElementInterface> =
      new Array<RcsbFvTrackDataElementInterface>();
    for (let n = where.from; n <= where.to; n++) {
      if (this.innerData[n]) addResToSeqPath(this.innerData[n], seqPath);
    }
    return seqPath;
  }
...
function addResToSeqPath(
  res: RcsbFvTrackDataElementInterface,
  seqPath: Array<RcsbFvTrackDataElementInterface>,
): void {
  if (
    seqPath.length > 0 &&
    seqPath[seqPath.length - 1].label!.length +
      seqPath[seqPath.length - 1].begin ==
      res.begin
  ) {
