...
export class RcsbCompositeDisplay implements RcsbDisplayInterface {
  private innerDisplays: Array<DisplayElementInterface> =
    new Array<DisplayElementInterface>();
  ...
  setUpdateDataOnMove: (
    f: (d: LocationViewInterface) => Promise<RcsbFvTrackData>,
  ) => void;
  ...
  addDisplay(displayId: string, display: RcsbDisplayInterface) {
    this.innerDisplays.push({
      id: displayId,
      display: display,
    } as DisplayElementInterface);
    this.trackSubject.mouseenter.subscribe((e) =>
      display.trackSubject.mouseenter.next(e),
    );
    this.trackSubject.mouseleave.subscribe((e) =>
      display.trackSubject.mouseleave.next(e),
    );
    this.trackSubject.mousemove.subscribe((e) =>
      display.trackSubject.mousemove.next(e),
    );
  }
  ...
  data(d?: RcsbFvTrackDataMap | RcsbFvTrackData): RcsbFvTrackDataMap {
    if (d != undefined) {
      ...
      this.innerDisplays.forEach((de) => {
        ...
        if (deData != undefined) de.display.data(deData);
      });
    }
    return this._data;
  }
