...
export class RcsbBoard {
  ...
  private tracks: Array<RcsbDisplayInterface> =
    new Array<RcsbDisplayInterface>();
  ...
  private readonly mouseHoverSubject: Subject<
    Array<RcsbFvTrackDataElementInterface>
  > = new Subject<Array<RcsbFvTrackDataElementInterface>>();
  public readonly elementClickSubject: Subject<{
  ...
  private readonly scrollEvent = (isIntersecting: boolean) => {
    ...
    if (!this.upToDate && this.isIntersecting) this.updateAndMove();
  };
  ...
  constructor(
    ...
    if (boardDiv == null) {
      throw (
        "Board DOM [" +
        this.domId +
        "] element not found. Removing scroll event handler from window"
      );
    }
  ...
  public addHoverCallback(
    f: (n: Array<RcsbFvTrackDataElementInterface>) => void,
  ) {
  ...
  public setSelection(boardId: string, mode: "select" | "hover"): void {
    if (this.domId != boardId) this.highlightRegion(null, "set", mode, true);
  }

  public highlightRegion(
    d: RcsbFvTrackDataElementInterface | null,
    operation: "set" | "add" | "replace-last",
    mode: "select" | "hover",
    propFlag?: boolean,
  ): void {
    if (d != null) {
  ...
  public startBoard(): void {
    if (
      this.currentLocationView.to - this.currentLocationView.from <
      this.limits.minZoom
    ) {
      this.currentLocationView.to =
        this.currentLocationView.from + this.limits.minZoom;
    } else if (
      this.currentLocationView.to - this.currentLocationView.from >
      this.limits.maxZoom
    ) {
      this.currentLocationView.to =
        this.currentLocationView.from + this.limits.maxZoom;
    }
    this.addSVG();
    if (
  ...
  private addHighlightHoverElement(t: RcsbDisplayInterface) {
    t.subscribeElementHighlight({
      enter: (d: RcsbFvTrackDataElementInterface) => {
        this.contextManager.setCondition(
          CONDITIONAL_FLAG.STOP_MOUSE_MOVE_HOVERING_HIGHLIGHT,
          true,
        );
  ...
  public addTrack(
    track: RcsbDisplayInterface | Array<RcsbDisplayInterface>,
    options?: {},
  ): void {
  ...
  private addTrackCallbacks(t: RcsbDisplayInterface) {
    ...
    if (this.highlightHoverElementFlag) this.addHighlightHoverElement(t);
    this.boardSubject.mouseleave.subscribe((e) =>
      t.trackSubject.mouseleave.next(e),
    );
    this.boardSubject.mouseenter.subscribe((e) =>
      t.trackSubject.mouseenter.next(e),
    );
    this.boardSubject.mousemove.subscribe((d) =>
      t.trackSubject.mousemove.next(d),
    );
    this.tracks.push(t);
  }
  ...
  private moveBoard(
    ...
    let newDomain: number[] = transform
      .rescaleX(this._xScale.getScale())
      .domain();
    ...
    if (length < this.limits.minZoom) {
      this.d3Manager
        .zoomG()
        .call(this.zoomEventHandler.transform, zoomIdentity);
      return;
    }
