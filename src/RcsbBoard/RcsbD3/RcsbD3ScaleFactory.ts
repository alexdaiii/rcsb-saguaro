...
export namespace RcsbD3ScaleFactory {
  export function getLinearScale(): RcsbScaleInterface {
    ...
    const scale: RcsbScaleInterface = ((x: number) =>
      xScale(x)) as RcsbScaleInterface;
    ...
    scale._domain = [0, 0];
    scale._range = [0, 0];
    scale.checkAndSetScale = (
      domain: [number, number],
      range: [number, number],
    ) => {
      if (
        domain[0] == scale._domain[0] &&
        domain[1] == scale._domain[1] &&
        range[0] == scale._range[0] &&
        range[1] == scale._range[1]
      ) {
        return true;
      } else {
  ...
  export function getPointScale(): RcsbScaleInterface<
    string,
    ScalePoint<string>
  > {
    const xScale: ScalePoint<string> = scalePoint();
    const scale: RcsbScaleInterface<string, ScalePoint<string>> = ((
      x: string,
    ) => xScale(x)) as RcsbScaleInterface<string, ScalePoint<string>>;
    scale.domain = xScale.domain as unknown as RcsbScaleInterface<
      string,
      ScalePoint<string>
    >["domain"];
    scale.range = xScale.range as unknown as RcsbScaleInterface<
      string,
      ScalePoint<string>
    >["range"];
    scale.getScale = () => xScale;
