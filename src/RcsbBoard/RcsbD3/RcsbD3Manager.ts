...
export class RcsbD3Manager {
  ...
  buildSvgNode(config: SVGConfInterface): void {
    ...
    this._dom
      .classed(config.domClass, true)
      .style(RcsbD3Constants.WIDTH, config.width + "px");

    this._svg = this._dom
      .append<SVGSVGElement>(RcsbD3Constants.SVG)
    ...
    this._svg
      ...
      .on(RcsbD3Constants.MOUSE_MOVE, (event) => {
        config.boardSubject.mousemove.next({
          e: event,
          n: Math.round(
            config.xScale.invert(pointer(event, this.getPane())[0]),
          ),
        });
      })
  ...
  getPane(): SVGRectElement {
    ...
    if (out == null) throw "SVG main panel is null";
    return out;
  }
  ...
  addTrack(
    config: TrackConfInterface,
  ): Selection<SVGGElement, any, null, undefined> {
    ...
    const trackG: Selection<SVGGElement, any, null, undefined> = this._svgG
      ...
      .attr(
        RcsbD3Constants.TRANSFORM,
        "translate(0," +
          (this._trackHeightPosition + config.compositeHeight) +
          ")",
      );
  ...
  highlightRegion(hlConfig: HighlightRegionInterface): void {
    const elementsToSelect: Array<SelectedElementInterface> =
      new Array<SelectedElementInterface>();
    const hlRegion: (b: number, e: number) => SelectedElementInterface = (
      begin: number,
      end: number,
    ) => {
    ...
    const minWidth = (begin: number, end: number) => {
      let w: number =
        (hlConfig.xScale(end + 0.5) ?? 0) - (hlConfig.xScale(begin - 0.5) ?? 0);
      if (w < 2) w = 2;
      return w;
    };
    ...
    const elements: Selection<
      ...
      undefined
    > = hlConfig.trackG
      .selectAll<SVGGElement, SelectedElementInterface>(
        "." + hlConfig.rectClass,
      )
      .data(elementsToSelect, (d) => {
  ...
  moveSelection(config: MoveSelectedRegionInterface): void {
    const minWidth = (begin: number, end: number) => {
      let w: number =
        (config.xScale(end + 0.5) ?? 0) - (config.xScale(begin - 0.5) ?? 0);
      if (w < 2) w = 2;
      return w;
    };
    const selectRect: Selection<
      SVGGElement,
      SelectedElementInterface,
      SVGElement,
      any
    > = config.trackG.selectAll<SVGGElement, any>("." + config.rectClass);
    selectRect
      .select(RcsbD3Constants.RECT)
      .attr(RcsbD3Constants.X, (d: SelectedElementInterface) => {
