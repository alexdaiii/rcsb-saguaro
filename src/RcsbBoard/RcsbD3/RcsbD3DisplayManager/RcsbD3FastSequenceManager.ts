...
export class RcsbD3FastSequenceManager {
  private textElements: Selection<
    SVGElement,
    RcsbFvTrackDataElementInterface,
    BaseType,
    undefined
  > = select<SVGElement, RcsbFvTrackDataElementInterface>(
  ...
  static plotSequenceLine(config: PlotFastSequenceLineInterface): void {
    ...
    const rect: Selection<SVGElement, any, any, any> = config.trackG.append(
      RcsbD3Constants.RECT,
    );
    rect
      .attr(RcsbD3Constants.ID, RcsbD3FastSequenceManager.RECT_ID)
      .attr(RcsbD3Constants.X, config.xScale.range()[0])
      .attr(RcsbD3Constants.Y, config.yScale.range()[0])
      .attr(
        RcsbD3Constants.WIDTH,
        config.xScale.range()[1] - config.xScale.range()[0],
      )
  ...
  public static clearLine(config: {
    trackG: Selection<SVGGElement, any, any, any>;
  }): void {
    config.trackG.select(RcsbD3Constants.LINE).remove();
    config.trackG
      .select(`${RcsbD3Constants.RECT}#${RcsbD3FastSequenceManager.RECT_ID}`)
      .remove();
  }
  ...
  move(config: MoveFastSequenceInterface) {
    asyncScheduler.schedule(() => {
      ...
      this.textElements
        ...
        .attr(RcsbD3Constants.FILL_OPACITY, () => {
          return RcsbD3FastSequenceManager.opacity(
            xScale,
            config.intervalRatio,
          );
        });
    });
  }
  ...
  private textLength(
    ...
    return (
      xScale(d.begin + d.label!.length - 1) -
      xScale(d.begin) +
      2 * this.MONOSPACE_BEGIN
    );
  }
...
function addSequenceEvents(
  element: Selection<SVGElement, any, any, any>,
  config: {
    mouseclick: (event: MouseEvent) => void;
    mousemove: (event: MouseEvent) => void;
    mouseleave: (event: MouseEvent) => void;
