...
export class RcsbD3BondManager {
  private beginCircleElements: Selection<
    SVGCircleElement,
    RcsbFvTrackDataElementInterface,
    BaseType,
    undefined
  ...
  private endCircleElements: Selection<
    SVGCircleElement,
    RcsbFvTrackDataElementInterface,
    BaseType,
    undefined
  ...
  plot(config: PlotBondInterface): void {
    ...
    this.lineElements
      ...
      .attr(RcsbD3Constants.X2, (d: RcsbFvTrackDataElementInterface) => {
        if (d.end == undefined) throw "Element end position not found";
    ...
    this.beginCircleElements = elements.select(
      RcsbD3Constants.CIRCLE + "." + RcsbD3Constants.BOND_BEGIN,
    );
    this.beginCircleElements
    ...
    this.endCircleElements = elements.select(
      RcsbD3Constants.CIRCLE + "." + RcsbD3Constants.BOND_END,
    );
    this.endCircleElements
      .attr(RcsbD3Constants.CX, (d: RcsbFvTrackDataElementInterface) => {
        if (d.end == undefined) throw "Element end position not found";
        return xScale(d.end) ?? 0;
      })
  ...
  move(config: MoveBondInterface): void {
    ...
    this.lineElements
      ...
      .attr(RcsbD3Constants.X2, (d: RcsbFvTrackDataElementInterface) => {
        if (d.end == undefined) throw "Missing bond end property";
        return xScale(d.end) ?? 0;
      });
