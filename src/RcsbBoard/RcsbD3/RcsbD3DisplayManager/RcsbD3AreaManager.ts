...
export class RcsbD3AreaManager {
  private areaMap: {
    [key: string]: Selection<SVGPathElement, any, null, undefined>;
  } = {};
    ...
    multiConfig.forEach((config) => {
      if (this.areaMap[config.id] == null) {
        ...
        this.areaMap[config.id]
          ...
          .style(RcsbD3Constants.FILL_OPACITY, config.opacity ?? 0.2)
  ...
  private addAreaEvents(
    area: Selection<SVGPathElement, any, null, undefined>,
    config: PlotAreaInterface,
  ) {
  ...
  public move(config: MoveAreaInterface) {
    config.trackG
      .select(RcsbD3Constants.PATH + "#" + config.id)
      .datum(config.points)
      .attr(RcsbD3Constants.D, config.area);
  }
}
