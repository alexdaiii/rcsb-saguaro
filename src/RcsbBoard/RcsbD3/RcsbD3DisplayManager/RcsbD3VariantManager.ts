...
export class RcsbD3VariantManager {
  private circleElements: Selection<
    ...
    BaseType,
    undefined
  > = select<SVGCircleElement, RcsbFvTrackDataElementInterface>(
  ...
  plot(config: PlotVariantInterface): void {
    ...
    this.circleElements
      .attr(RcsbD3Constants.CX, (d: RcsbFvTrackDataElementInterface) => {
        if (d.begin == undefined) throw "Position element not found";
        return config.xScale(d.begin) ?? 0;
      })
      .attr(RcsbD3Constants.CY, (d: RcsbFvTrackDataElementInterface) => {
        if (d.label == undefined) throw "Variant value not found";
    ...
    RcsbD3VariantManager.includeAxis(
      config.trackG,
      config.xScale,
      config.yScale,
      config.height,
  ...
  private static includeAxis(
    trackG: Selection<SVGGElement, any, BaseType, undefined>,
    xScale: RcsbScaleInterface,
    yScale: RcsbScaleInterface<string, ScalePoint<string>>,
    height: number,
  ) {
    ...
    yScale.domain().forEach((aa: string) => {
      ...
      if (aaY != undefined)
        trackG
          .selectAll("." + classes.rcsbVariantGrid)
      ...
      else console.warn("Variation (" + aa + ") not found");
    });
    trackG
      .selectAll<SVGGElement, RcsbFvTrackDataElementInterface>(
        "." + classes.rcsbElement,
      )
      .each(function () {
    ...
    const gAxis: Selection<SVGGElement, any, BaseType, undefined> =
      trackG.append(RcsbD3Constants.G);
    gAxis
      .classed(classes.rcsbAxis, true)
