...
interface RcsbFvBlockDataElementInterface
  extends RcsbFvTrackDataElementInterface,
    BlockElementInterface {}

export interface PlotBlockInterface {
  elements: Selection<
    SVGGElement,
    RcsbFvBlockDataElementInterface,
    BaseType,
    undefined
  >;
  dy: number;
...
export class RcsbD3BlockManager {
  ...
  private rectElements: Selection<
    SVGRectElement,
    RcsbFvBlockDataElementInterface,
    BaseType,
    undefined
  > = select<SVGRectElement, RcsbFvBlockDataElementInterface>(
  ...
  private lineElements: Selection<
    ...
    BaseType,
    undefined
  > = select<SVGLineElement, LineDecoratorInterface>(RcsbD3Constants.EMPTY);
  private circleElements: Selection<
    SVGCircleElement,
    CircleDecoratorInterface,
    BaseType,
    undefined
  > = select<SVGCircleElement, CircleDecoratorInterface>(RcsbD3Constants.EMPTY);

  private readonly STROKE_WIDTH: number = 1;

  plot(config: PlotBlockInterface): void {
    this.rectElements = config.elements.select<SVGRectElement>(
      RcsbD3Constants.RECT,
    );
    this.rectElements
      ...
      .attr(RcsbD3Constants.WIDTH, (d: RcsbFvBlockDataElementInterface) => {
        if (d.end != null) {
          ...
          return (
            RcsbD3BlockManager.getMinWidth(
              begin,
              end,
              config.xScale,
              config.dx,
        ...
        } else return RcsbD3BlockManager.minWidth;
      })
  ...
  private plotCircles(config: PlotCircleInterface): void {
    this.circleElements = config.elements.select<SVGCircleElement>(
      RcsbD3Constants.CIRCLE,
    );
    this.circleElements
      .attr(RcsbD3Constants.CX, (d: CircleDecoratorInterface) => {
  ...
  private plotLine(config: PlotLineInterface): void {
    this.lineElements = config.elements.select<SVGLineElement>(
      RcsbD3Constants.LINE,
    );
    this.lineElements
  ...
  private moveBlock(xScale: RcsbScaleInterface, dx: number): void {
    this.rectElements
      .attr(RcsbD3Constants.X, (d: RcsbFvBlockDataElementInterface) => {
        const begin: number = d.rectBegin ?? d.begin;
      ...
      .attr(RcsbD3Constants.WIDTH, (d: RcsbFvBlockDataElementInterface) => {
        if (d.end != null) {
          ...
          const end: number = d.rectEnd ?? d.end;
          return (
            RcsbD3BlockManager.getMinWidth(begin, end, xScale, dx) -
            this.STROKE_WIDTH
          );
        } else {
  ...
  private moveCircle(xScale: RcsbScaleInterface, dx: number): void {
    this.circleElements.attr(
      RcsbD3Constants.CX,
      (d: CircleDecoratorInterface) => {
